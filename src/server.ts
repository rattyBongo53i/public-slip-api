...
interface MasterSlip {
  _id?: ObjectId;
  master_slip_id: string;
  id?: number; // Laravel ID
  user_id: string;
  stake: number;
  currency?: string;
  total_odds: number;
...
interface GeneratedSlipLeg {
  match_id: number;
  match?: Match; // Populated match data
  selection: string;
  odds: number;
  market: string;
}
...
interface GeneratedSlip {
  ...
  risk_level?: string;
  risk_category?: string; // Alternative to risk_level
  generated_at?: Date;
  created_at?: Date;
  updated_at?: Date;
  status?: "active" | "won" | "lost" | "void";
  estimated_return?: number;
  estimated_payout?: number; // Alternative field name
  possible_return?: number; // Alternative to estimated_return
  diversity_score?: number;
  slip_data?: any;
}

interface OptimizedSlip {
  _id?: ObjectId;
  id: number;
  master_slip_id: number;
  stake?: number;
...
interface DatabaseCollections {
  ...
  matches?: Collection<Match>; // Optional matches collection
}
...
async function connectMongo(): Promise<void> {
  ...
  try {
    ...
    collections = {
      ...
      matches: db.collection<Match>("matches"), // Add matches collection
    };
...
// ---- Guard helper ----
function requireCollectionsOrFail(res: express.Response) {
  if (!collections) {
...
app.get("/api/placement-slips/:masterSlipId", async (req, res) => {
  if (!requireCollectionsOrFail(res)) return;
...
app.get("/api/master-slips/:masterSlipId", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Error fetching master slip ${req.params.masterSlipId}:`,
      err,
    );
    res.status(500).json({
      success: false,
...
app.patch("/api/master-slips/:masterSlipId", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Error updating master slip ${req.params.masterSlipId}:`,
      err,
    );
    res
      .status(500)
...
app.get("/api/master-slips/:masterSlipId/generated-slips", async (req, res) => {
  ...
  try {
    ...
    if (status) query.status = String(status);
    if (risk_level) query.risk_level = String(risk_level);
    ...
    console.error(
      `Error fetching generated slips for master slip ${req.params.masterSlipId}:`,
      err,
    );
    res.status(500).json({
      success: false,
...
app.post(
  "/api/master-slips/:masterSlipId/generated-slips",
  async (req, res) => {
    if (!requireCollectionsOrFail(res)) return;
    ...
    try {
      ...
      const result =
        await collections!.generated_slips.insertMany(slipsToInsert);
...
app.delete("/api/master-slips/:masterSlipId/slips", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Error deleting slips for master slip ${req.params.masterSlipId}:`,
      err,
    );
...
async function startServer(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(
        `📊 Attempted DB: ${maskMongoUri(MONGO_URI)} (db: ${MONGO_DB_NAME})`,
      );
