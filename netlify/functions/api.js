...
function maskMongoUri(uri) {
    try {
        return uri.replace(/(\/\/)([^:@]+):([^@]+)@/, (_m, p1, u, _p) => `${p1}${u}:****@`);
    }
    catch {
        return uri;
    }
...
function respondError(statusCode, error, message) {
...
async function handleGetPlacementSlips(masterSlipId) {
    if (!collections) {
    ...
    try {
        ...
        console.error(`Error fetching placement slips for master slip ${masterSlipId}:`, err);
        return respondError(500, "Failed to fetch placement slips", err.message);
    }
}
async function handleGetMasterSlips(ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
        ...
        if (ctx.query.user_id)
            query.user_id = String(ctx.query.user_id);
        if (ctx.query.status)
            query.status = String(ctx.query.status);
...
async function handleGetMasterSlip(masterSlipId) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleCreateMasterSlip(ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleUpdateMasterSlip(masterSlipId, ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleGetGeneratedSlips(masterSlipId, ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
        ...
        if (ctx.query.status)
            query.status = String(ctx.query.status);
        if (ctx.query.risk_level)
            query.risk_level = String(ctx.query.risk_level);
        ...
        console.error(`Error fetching generated slips for master slip ${masterSlipId}:`, err);
        return respondError(500, "Failed to fetch generated slips");
    }
}
async function handleGetGeneratedSlip(slipId) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleCreateGeneratedSlips(masterSlipId, ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
        ...
        await collections.master_slips.updateOne({ master_slip_id: masterSlipId }, {
...
async function handleUpdateSlipStatus(slipId, ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleDeleteSlips(masterSlipId) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleSyncSlips(ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
async function handleGetMasterSlipsWithSlips(ctx) {
    if (!collections) {
        return respondError(503, "Database unavailable");
    }
    try {
...
export async function handler(event, _context) {
    ...
    const genSlipsMatch = ctx.path.match(/^\/api\/master-slips\/([^/]+)\/generated-slips$/);
    ...
    // Update slip status
    const statusMatch = ctx.path.match(/^\/api\/generated-slips\/([^/]+)\/status$/);
